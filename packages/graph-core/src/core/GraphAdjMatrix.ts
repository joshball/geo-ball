...
export class GraphAdjMatrix extends Graph {
    ...
    constructor(startingMatrixDim: number = 10, matrixGrowthFactor: number = 2) {
        super();
    ...
    createMatrix(newSize: number, oldMatrix?: Array<Array<number>>): Array<Array<number>> {
        const newAdjMatrix = new Array<Array<number>>();
        const getNewValue = (i: number, j: number): number => {
    ...
    getNeighbors(v: number): Array<number> {
        ...
        const neigbors = new Array<number>();
