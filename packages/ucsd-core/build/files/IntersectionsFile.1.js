...
class IntersectionsFile {
    constructor(metaData, segmentsData) {
        this.metaData = new RoadSegmentsFileMetaData(metaData.bounds, metaData.timestamp);
        this.segmentsData = segmentsData;
    }
    static CreateFromOsm(osmFile) {
        const metaData = IntersectionsFile.CreateMetaDataFromOsm(osmFile);
        const segmentsData = IntersectionsFile.CreateSegmentsDataFromOsm(osmFile);
        return new IntersectionsFile(metaData, segmentsData);
    }
    ...
    static CreateSegmentsDataFromOsm(_osmFile) {
        ...
        ways
            ...
            for (let i = 0; i < nodeIds.length - 1; i++) {
                const start = IntersectionsFile.DeReferenceNode(nodes, nodeIds[i]);
                const startLatLng = new geo_core_1.LatLng(start.lat, start.lon);
                const end = IntersectionsFile.DeReferenceNode(nodes, nodeIds[i + 1]);
    ...
    static CreateMetaDataFromOsm(_osmFile) {
        return new RoadSegmentsFileMetaData(_osmFile.osmMetaData.osmQuery.latLngBounds, new Date(_osmFile.osmMetaData.queryDate));
    }
    ...
    static LoadFromJsonFile(filePath) {
        ...
        return new IntersectionsFile(file.metaData, file.segmentsData);
    }
...
//# sourceMappingURL=IntersectionsFile.1.js.map