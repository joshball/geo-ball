...
export interface IOverpassQuery {
...
export class OverpassQuery {
    ...
    constructor(osmQueryObj: IOverpassQuery) {
        ...
        if (osmQueryObj.features) {
            this.features = osmQueryObj.features.map(
                (kvp: OSMFeatureKeyValuePair) =>
                    new OSMFeatureKeyValuePair(kvp.key, kvp.values)
            );
        } else {
            const defaultFeatureKeys = ['highway', 'addr'] as Array<
                OSMFeatureKey
            >;
            this.features = defaultFeatureKeys.map(
                key => new OSMFeatureKeyValuePair(key)
            );
        }
    }
        ...
        this.bds.forEach(bds => {
            this.features.forEach(kvp => {
                kvp.values.forEach(value => {
                    ...
                    featuresStrings.push(
                        `    ${bds}["${kvp.key}${rest}"]${boundsString};`
                    );
                });
            });
        });
