...
Fenced code blocks with `ts`, `tsx` or `javascript` languages are rendered as a interactive playgrounds:

```tsx
<Button>Push Me</Button>
```

You can add a custom props to an example wrapper (` ```ts { "props": { "className": "checks" } } `):

```ts { "props": { "className": "checks" } }
<Button>I’m transparent!</Button>
```

Or disable an editor by passing a `noeditor` modifier (` ```ts noeditor `):

```tsx noeditor
<Button>Push Me</Button>
```

To render an example as highlighted source code add a `static` modifier: (` ```ts static `):
...
```tsx
import React from 'react'
import Button from 'rsg-example/components/atoms/Button'
import Placeholder from 'rsg-example/components/atoms/Placeholder'
;<Button>
  <Placeholder />
</Button>
```
