...
import { IOsmFetchDir, OsmFetchDir } from '../../services/fetch/OsmFetchDir';
import { IOsmFetchFileMeta } from '../../services/fetch/OsmFetchFile';

interface OsmOtherFilesComponentProps {
    files: Array<string>;
}

export const OsmOtherFilesComponent: React.SFC<OsmOtherFilesComponentProps> = (
    props: OsmOtherFilesComponentProps
) => (
    <div>
        <h1>Other Files</h1>
        <div>
...
export const OsmFetchesComponent: React.SFC<OsmFetchesComponentProps> = (
    props: OsmFetchesComponentProps
) => {
    console.log('OsmFetchesComponent fetches', props.fetches);
    const fetches = props.fetches
        ? props.fetches.map((f, i) => <div key={i}>{f.dirName}</div>)
        : 'NONE';
    return (
        <div>
            <h1>All Downloads</h1>
...
export const OsmFetchComponent: React.SFC<OsmFetchComponentProps> = (
    props: OsmFetchComponentProps
) => {
...
export const OsmFileDetailComponent: React.SFC<OsmFileDetailComponentProps> = (
    props: OsmFileDetailComponentProps
) => (
    <div>
        <h1>File Details</h1>
...
export const OsmQueryDetailComponent: React.SFC<
    OsmQueryDetailComponentProps
> = (props: OsmQueryDetailComponentProps) => (
    <div>
        <h1>Query Details</h1>
        <h3>Name: {props.name}</h3>
...
// const fullPageGridCss = css(
//     { display: 'grid', gridTemplateColumns: '300px 1fr', width: '100%' },
//     fullHeightCss,
// );
// const mainContainerGridCss = css(
//     { display: 'grid', gridTemplateRows: '60% 40%', width: '100%' },
//     fullHeightCss,
// );
...
export const OsmFetchManagerView: React.SFC<OsmFetchManagerViewProps> = (
    props: OsmFetchManagerViewProps
) => {
    ...
    const allRunsComponent = (
        <OsmFetchesComponent fetches={props.allOsmFetchDirs} />
    );
    ...
    //     <div {...css(mainContainerGridCss)}>
    //         {mainTopDetailDiv}
    //         {mainBottomDetailDiv}
    //     </div>
    // );
    // const leftSideDiv = <div {...css(boxCss, greenCss)}>{allRunsComponent}</div>;
    // const fullContentDiv = (
    //     <div {...css(fullPageGridCss)}>
    //         {leftSideDiv}
    //         {mainContentDiv}
    //     </div>
    // );
...
export class OsmFetchManagerComponent extends React.Component<
    OsmFetchManagerComponentProps,
    OsmFetchManagerComponentState
> {
    ...
    async refreshFiles(): Promise<void> {
        ...
        // return (
        //     getOsmFetchData(this.state.osmFetchDirPath)
        //         .then(osmFetchMgr => {
