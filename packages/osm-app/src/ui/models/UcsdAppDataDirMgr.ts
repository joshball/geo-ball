...
export class UcsdAppDataDirMgr implements IUcsdAppDataDirMgr {
    ...
    set fullPath(path: string) {
        const {
            rootPath,
            dirName,
        } = UcsdAppDataDirMgr.GetUcsdAppDataDirMgrFromPath(path);
    ...
    get osmFetchDirPath(): string {
        return `${join(
            this.fullPath,
            UcsdAppDataDirMgr.DEFAULT_OSM_FETCHES_DIRNAME
        )}`;
    }
    ...
    constructor(...pathArgs: Array<string>) {
        const {
            rootPath,
            dirName,
        } = UcsdAppDataDirMgr.GetUcsdAppDataDirMgrFromPath(...pathArgs);
        this.rootPath = rootPath;
        this.dirName = dirName;
    }
     ...
     */
    static GetUcsdAppDataDirMgrFromPath(
