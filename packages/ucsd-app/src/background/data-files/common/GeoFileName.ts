...
export class GeoFileName implements IGeoFileName {
    ...
    static SplitGeoFileName = (fileName: string): Array<string> => {
        ...
        if (split.length !== 4) {
            throw new OsmRunError('filename split is not 4', 'DirHasInconsistentFileNames');
        }
        return split;
    }
...
export const fileDateStrToDate = (dateString: string): Date => {
    return parseDate(dateString, FILE_DATE_STR, new Date());
}
...
export const dataTypeStrToDataType = (dataStr: string): GeoFileDataType => {
    switch (dataStr) {
        ...
        case 'text':
            return 'TEXT';
        default:
            throw new OsmRunError(`Unknown data file type: [${dataStr}]`, 'DirHasInconsistentFileNames');
    }
}
...
export const geoFileTypeStrToGeoType = (typeString: string): GeoFileType => {
    switch (typeString) {
        ...
        default:
            throw new OsmRunError(`Unknown geo file type: [${typeString}]`, 'DirHasInconsistentFileNames');
    }
}
...
export const geoFileTypeToStr = (type: GeoFileType): string => {
    switch (type) {
        ...
        case 'ROAD_SEGMENTS':
            return 'rsd';
        default:
            throw new OsmRunError(`Unknown geo file type: [${type}]`, 'DirHasInconsistentFileNames');
    }
}

export const dateToFileDateStr = (date: Date): string => {
    return formatDate(date, FILE_DATE_STR);
}
